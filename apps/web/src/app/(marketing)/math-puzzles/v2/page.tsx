...
import { useCallback, useEffect, useMemo, useState, type DragEvent, type TouchEvent } from "react";
...
const METAS: PuzzleMeta[] = [
  ["weightScale", "Weight Scale", "Solve hidden weights", "⚖️", "from-amber-400 to-orange-500"],
  ["fractionPizza", "Fraction Pizza", "Build and compare slices", "🍕", "from-rose-400 to-red-500"],
  ["numberMachine", "Number Machine", "Find the hidden rule", "⚙️", "from-sky-400 to-blue-600"],
  ["patternBlocks", "Pattern Blocks", "Continue the sequence", "🧩", "from-violet-400 to-fuchsia-600"],
  ["areaBuilder", "Area Builder", "Count tiles and edges", "🟩", "from-emerald-400 to-green-600"],
  ["gridPath", "Grid Path", "Walk through operations", "🎯", "from-cyan-400 to-teal-600"],
  ["waterFill", "Water Fill Puzzle", "Pour to the target", "💧", "from-blue-400 to-cyan-600"],
  ["treasureEquations", "Treasure Chest Equations", "Crack the lock", "💎", "from-yellow-300 to-amber-600"],
  ["monsterMerge", "Monster Merge Math", "Combine creature values", "👾", "from-lime-400 to-emerald-600"],
  ["clock", "Clock Puzzle", "Read the time", "🕒", "from-indigo-400 to-purple-600"],
  ["balanceBeam", "Balance Beam Puzzle", "Level the beam", "🪵", "from-orange-300 to-stone-600"],
  ["diceProbability", "Dice Probability", "Predict the chance", "🎲", "from-red-400 to-pink-600"],
  ["coordinateTreasure", "Coordinate Treasure Hunt", "Find the point", "🗺️", "from-teal-400 to-cyan-700"],
  ["shapeFolding", "Shape Folding Puzzle", "Fold in your mind", "📦", "from-fuchsia-400 to-violet-700"],
  ["lightBeam", "Light Beam Reflection", "Aim the mirror", "🔦", "from-yellow-200 to-sky-500"],
  ["bridgeWeight", "Bridge Weight Puzzle", "Stay under limit", "🌉", "from-slate-300 to-slate-700"],
  ["resourceManagement", "Resource Management", "Spend wisely", "🪙", "from-amber-300 to-lime-600"],
  ["numberPyramid", "Missing Number Pyramid", "Build upward", "🔺", "from-red-300 to-orange-600"],
  ["snakePath", "Arithmetic Snake Path", "Draw the value trail", "🐍", "from-green-400 to-teal-700"],
  ["equationMatch", "Equation Match Puzzle", "Pair equations", "🔗", "from-blue-400 to-indigo-700"],
  ["multiplicationArray", "Multiplication Array", "See multiplication", "🔢", "from-emerald-300 to-sky-600"],
  ["sudokuMini", "Sudoku Mini", "Tiny logic grid", "🧠", "from-purple-300 to-indigo-700"],
  ["magicSquare", "Magic Square Puzzle", "Rows share totals", "✨", "from-yellow-300 to-purple-600"],
  ["memoryMatch", "Math Memory Match", "Find equivalents", "🃏", "from-pink-300 to-rose-700"],
  ["tangram", "Tangram Geometry Puzzle", "Arrange the pieces", "🔷", "from-cyan-300 to-blue-700"],
  ["primeCatcher", "Prime Number Catcher", "Catch only primes", "⭐", "from-violet-300 to-pink-600"],
  ["fractionBars", "Fraction Bar Comparison", "Compare lengths", "📊", "from-orange-300 to-red-600"],
  ["decimalSlider", "Decimal Slider Puzzle", "Tune the value", "🎚️", "from-sky-300 to-cyan-700"],
  ["ratioRecipe", "Ratio Recipe Puzzle", "Mix the recipe", "🥣", "from-amber-300 to-rose-600"],
  ["escapeRoom", "Math Escape Room", "Unlock every door", "🚪", "from-zinc-300 to-violet-700"],
].map(([id, title, short, emoji, gradient]) => ({ id, title, short, emoji, gradient })) as PuzzleMeta[];
...
export default function MathPuzzlesPage() {
  ...
  const difficulty = difficultyFor(solved);
  ...
  const start = useCallback(
    (type: PuzzleId) => {
      const next = makePuzzle(type, difficulty);
      setActive(type);
      setPuzzle(next);
  ...
  const nextPuzzle = useCallback(() => {
    if (!active) return;
    const next = makePuzzle(active, difficultyFor(solved + 1));
    setPuzzle(next);
    setRawState(initialState(next));
    setResult("idle");
    setHint(false);
  ...
  const check = () => {
    ...
    if (correct) {
      setXp((value) => value + XP_PER_WIN + (difficulty === "hard" ? 8 : difficulty === "medium" ? 4 : 0));
          ...
          {!active ? (
            <motion.section key="selector" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
              ...
              <div className="mb-4 flex items-end justify-between">
                ...
                <span className="rounded-full bg-white/5 px-3 py-1 text-xs font-bold capitalize text-zinc-300">{difficulty}</span>
              </div>

              <div className="-mx-4 overflow-x-auto px-4 pb-2">
                <div className="grid min-w-[58rem] grid-flow-col grid-rows-3 gap-3">
                  {METAS.map((meta, i) => (
                    <motion.button key={meta.id} type="button" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.015 }} whileTap={{ scale: 0.97 }} onClick={() => start(meta.id)} className="relative h-32 w-44 overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/75 p-4 text-left shadow-xl ring-1 ring-white/5">
                      <div className={`absolute -right-8 -top-8 grid h-24 w-24 place-items-center rounded-full bg-gradient-to-br ${meta.gradient} text-4xl shadow-lg`}>{meta.emoji}</div>
                      <div className="absolute bottom-4 left-4 right-4">
                        <h3 className="text-sm font-black leading-tight text-white">{meta.title}</h3>
                        <p className="mt-1 text-xs leading-tight text-zinc-400">{meta.short}</p>
                      </div>
                    </motion.button>
                  ))}
                </div>
