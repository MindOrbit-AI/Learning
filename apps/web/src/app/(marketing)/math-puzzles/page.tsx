...
function CategoryCard({ meta, xp, onClick }: { meta: PuzzleMeta; xp: number; onClick: () => void }) {
  return (
    <motion.button
      type="button"
      onClick={onClick}
      whileTap={{ scale: 0.97 }}
      className="group relative flex h-full flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950/70 p-4 text-left shadow-xl shadow-black/20 ring-1 ring-white/5 transition hover:-translate-y-0.5 hover:border-white/20 hover:bg-zinc-900/85"
    >
      <div className={`pointer-events-none absolute inset-x-0 top-0 h-24 bg-gradient-to-br ${meta.gradient} opacity-10 transition group-hover:opacity-20`} />
      <div className="flex items-start gap-3">
        <div className={`relative grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-gradient-to-br ${meta.gradient} text-3xl shadow-lg`}>
          {meta.emoji}
        </div>
        <div className="min-w-0 flex-1">
          <h3 className="line-clamp-2 text-sm font-black leading-tight text-white">{meta.title}</h3>
...
export default function MathPuzzlesPage() {
  ...
  const visibleMetas = useMemo(
    ...
    [gradeFilter, subjectFilter, interactionFilter, search],
  );

  useEffect(() => {
    const saved = window.localStorage.getItem(STORAGE_KEY);
    if (saved) {
      const progress = JSON.parse(saved) as { xp?: number; streak?: number; solved?: number; mistakes?: PuzzleId[] };
      setXp(progress.xp ?? 0);
  ...
  useEffect(() => {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ xp, streak, solved, mistakes: mistakeIds }));
  }, [xp, streak, solved, mistakeIds]);
  ...
  const start = useCallback(
    (type: PuzzleId, modeOverride?: GameMode) => {
      ...
      setRunOver(false);
      const meta = GAME_MODE_META[mode];
      setSecondsLeft(meta.countdown ?? 0);
      if (mode === "survival") setLives(meta.lives ?? 3);
  ...
  }, [active, gameMode, masteryRemaining, mistakeIds, resolveDifficulty, solved]);
  ...
  const subjectChips = useMemo(() => {
    return SUBJECT_OPTIONS.filter((option) => option !== "All").map((subject) => ({
      subject,
  ...
  const randomStart = () => {
    const pool = visibleMetas.length > 0 ? visibleMetas : METAS;
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-black/55 px-4 pb-3 pt-[max(0.75rem,env(safe-area-inset-top))] backdrop-blur-2xl">
        <div className="mx-auto max-w-7xl">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="truncate text-lg font-black tracking-tight sm:text-xl">{activeMeta?.title ?? "Math Puzzles"}</h1>
              <p className="truncate text-xs text-zinc-400">{activeMeta?.short ?? "150 puzzle arcade · Grades K–12"}</p>
            </div>
            ...
            <motion.section key="selector" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="space-y-6">
              <section className="overflow-hidden rounded-[2.5rem] border border-white/10 bg-zinc-950/70 shadow-2xl shadow-black/30 ring-1 ring-white/5">
                <div className="relative grid gap-6 p-5 sm:p-6 lg:grid-cols-[1.25fr_0.75fr] lg:p-8">
                  ...
                  <div className="relative">
                    <p className="text-xs font-black uppercase tracking-[0.24em] text-violet-200">Visual Math Arcade</p>
                    ...
                    <p className="mt-4 max-w-xl text-sm leading-6 text-zinc-300 sm:text-base">
                      A polished playground for 150 visual puzzles across K–12, with filters, modes, streaks, XP, and fast playable samples.
                    </p>
                  ...
                  {visibleMetas.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                      {visibleMetas.map((meta, i) => (
                        ...
                        >
                          <CategoryCard meta={meta} xp={xpRewardFor(effectiveDifficulty)} onClick={() => start(meta.id)} />
                        </motion.div>
                      ))}
                    </div>
                  )}
