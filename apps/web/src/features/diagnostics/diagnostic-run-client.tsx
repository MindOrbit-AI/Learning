...
export type DiagnosticRunClientProps = {
  /** e.g. `/api/diagnostics` or `/api/marketing/diagnostics` */
  diagnosticsApiBase: string;
  resultsPathForAttempt: (subjectSlug: string, attemptId: string) => string;
  backToSubjectHref: (subjectSlug: string) => string;
  /** When false, 403 is treated like other errors (redirect back). */
  showUpgradeOnLimit?: boolean;
};

export function DiagnosticRunClient({
  diagnosticsApiBase,
  resultsPathForAttempt,
  backToSubjectHref,
  showUpgradeOnLimit = true,
}: DiagnosticRunClientProps) {
  ...
  const backToSubjectHrefRef = useRef(backToSubjectHref);
  backToSubjectHrefRef.current = backToSubjectHref;
  const resultsPathRef = useRef(resultsPathForAttempt);
  resultsPathRef.current = resultsPathForAttempt;

  useEffect(() => {
    if (!subjectSlug) return;
    async function init() {
      const res = await fetch(`${diagnosticsApiBase}/${subjectSlug}/start`, {
        method: "POST",
      });
      const data = await res.json();
      if (!res.ok) {
        if (showUpgradeOnLimit && res.status === 403 && data.upgradeRequired) {
          ...
          router.push(backToSubjectHrefRef.current(subjectSlug));
        }
        setLoading(false);
        return;
      }
    ...
    init();
  }, [subjectSlug, router, diagnosticsApiBase, showUpgradeOnLimit]);

  async function handleSubmit() {
    if (!attemptId || !subjectSlug) return;
    const res = await fetch(`${diagnosticsApiBase}/${subjectSlug}/submit`, {
      method: "POST",
    ...
    const data = await res.json();
    router.push(resultsPathRef.current(subjectSlug, data.attemptId));
  }
    ...
    return (
      <div className="mx-auto max-w-md space-y-4 rounded-2xl border border-muted p-8 text-center">
        ...
        <Button variant="outline" asChild>
          <Link href={backToSubjectHref(subjectSlug)}>Back</Link>
        </Button>
      </div>
    );
  }
