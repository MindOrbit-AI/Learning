...
import type { CgeoLessonSeed } from "@mindorbit/content";
...
function primitivesForSeed(seed: CgeoLessonSeed): InteractiveCatalogItem["primitives"] {
...
function sceneCategoriesForSeed(seed: CgeoLessonSeed): InteractiveCatalogItem["sceneCategories"] {
...
export function cgeoLessonToCatalogItem(seed: CgeoLessonSeed): InteractiveCatalogItem {
  const style = LEVEL_ACCENT[seed.cgeoTrackLevel] ?? LEVEL_ACCENT[1]!;
  return {
    id: seed.id,
    title: seed.title,
    subject: "Algebra",
...
export function buildCoordinateGeometryCatalogItems(seeds: CgeoLessonSeed[]): InteractiveCatalogItem[] {
  return seeds.map(cgeoLessonToCatalogItem);
}
